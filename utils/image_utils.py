...
def load_mask(filepath, aug=False):
    img = cv2.imread(filepath, cv2.IMREAD_GRAYSCALE)
    if aug:
        random_number = np.random.randint(-10, 11)
        if random_number < 0:
            img = cv2.erode(img, np.ones((-random_number, -random_number), np.uint8))
        elif random_number > 0:
            img = cv2.dilate(img, np.ones((random_number, random_number), np.uint8))
...
def save_img(img, filepath):
    ...
    if filepath.endswith('.png'):
        ...
        cv2.imwrite(filepath, cv2.cvtColor(img, cv2.COLOR_RGB2BGR), [int(cv2.IMWRITE_JPEG_QUALITY), 100])
