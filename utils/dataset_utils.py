...
class Augment_RGB_torch:
    ...
    def transform8(self, torch_tensor):
        if(torch_tensor.size(0) != 3):
            return torch_tensor
        idx = torch.randperm(3)
        torch_tensor = torch_tensor[idx]
        return torch_tensor
...
class MixUp_AUG:
    ...
    def aug(self, rgb_gt, rgb_noisy, gray_mask, boundary):
        bs = rgb_gt.size(0)
        indices = torch.randperm(bs)
        rgb_gt2 = rgb_gt[indices]
        rgb_noisy2 = rgb_noisy[indices]
        gray_mask2 = gray_mask[indices]
        boundary2 = boundary[indices]
        ...
        boundary = lam * boundary + (1-lam) * boundary2
        ...
        return rgb_gt, rgb_noisy, gray_mask, boundary


def rgb_shuffle(rgb_gt, rgb_noisy):
    perm = np.random.permutation(3)
    rgb_gt = rgb_gt[:, perm]
    rgb_noisy = rgb_noisy[:, perm]
